...
export function Hero({ dict, lang }: HeroProps) {
  return (
    <section className="relative overflow-hidden bg-gradient-to-br from-[#0f172a] via-slate-900 to-[#0f172a] dark:bg-gradient-to-br dark:from-[#0f172a] dark:via-slate-900 dark:to-[#0f172a]">
      {/* Light theme: subtle warm gradient */}
      <div className="absolute inset-0 bg-gradient-to-br from-[#f8fafc] via-[#f1f5f9] to-[#e2e8f0] dark:hidden" />
      
      {/* Dot grid background - dark theme */}
      <div className="absolute inset-0 opacity-[0.07] dark:opacity-[0.07] hidden dark:block">
        <div
      ...
      <div className="absolute top-20 right-20 w-72 h-72 bg-[#f59e0b]/10 rounded-full blur-[100px] dark:bg-[#f59e0b]/10 hidden dark:block" />
      <div className="absolute bottom-10 left-10 w-56 h-56 bg-[#3b82f6]/10 rounded-full blur-[80px] dark:bg-[#3b82f6]/10 hidden dark:block" />

      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 relative py-16 md:py-24">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {/* Heading */}
          <h1 className="text-4xl md:text-6xl font-bold text-white dark:text-white mb-6 leading-tight tracking-tight">
            {dict.hero.title1}
            <br />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-3xl mx-auto mt-20">
          {dict.hero.stats.map((stat) => (
            <div key={stat.label} className="text-center">
              ...
              <div className="text-slate-400 dark:text-slate-400 text-sm">{stat.label}</div>
            </div>
          ))}
        </div>
      </div>
