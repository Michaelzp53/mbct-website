...
export default function HeroWithStats({ lang }: HeroWithStatsProps) {
  const isZh = lang === 'zh'
  const entryPoints = [
    { icon: Building2, label: isZh ? '准备投资' : 'Planning an investment', href: `/${lang}/knowledge?topic=investment#topic-results` },
    { icon: ClipboardCheck, label: isZh ? '正在筹开' : 'Preparing to open', href: `/${lang}/knowledge?topic=hotel-opening#topic-results` },
    { icon: RefreshCw, label: isZh ? '经营改善' : 'Improving operations', href: `/${lang}/knowledge?topic=operations#topic-results` },
  ]

  return (
    <section className="relative min-h-[440px] overflow-hidden border-b border-slate-200 bg-[#17231f] text-white dark:border-slate-800 lg:min-h-[520px]">
      <Image
      ...
      <div className="relative mx-auto flex min-h-[440px] max-w-[1440px] flex-col justify-center px-5 py-10 sm:px-8 lg:min-h-[520px] lg:px-12 xl:px-16">
        <div className="min-w-0 max-w-[820px]">
          <p className="mb-4 text-sm font-semibold leading-6 text-amber-300 sm:text-base">
          ...
          <h1 className={`max-w-full font-semibold leading-[1.12] text-white [overflow-wrap:anywhere] ${isZh ? 'text-[38px] sm:text-[52px] lg:text-[58px]' : 'text-[36px] sm:text-5xl lg:text-[54px]'}`}>
            {isZh ? '让酒店经营问题，有判断、有方法' : 'Practical answers to hotel operating questions'}
          </h1>
          <p className="mt-5 max-w-[720px] text-base leading-7 text-slate-100 sm:text-lg sm:leading-8">
            {isZh
          ...
          <div className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center">
            <Link
              href={`/${lang}/knowledge#problem-navigation`}
            ...
            >
              {isZh ? '查找经营问题与方法' : 'Find answers and methods'}
            ...
            >
              {isZh ? '阅读精益管理方法' : 'Explore lean management'}
              <ArrowRight className="h-5 w-5" />
            </Link>
          </div>
        </div>

        <div className="mt-7 grid max-w-[820px] border-y border-white/25 sm:grid-cols-3">
          {entryPoints.map(({ icon: Icon, label, href }) => (
            <Link key={label} href={href} className="group flex min-h-12 items-center justify-between gap-3 border-b border-white/25 px-1 py-3 text-sm font-semibold text-white transition-colors hover:text-amber-300 sm:border-r sm:border-b-0 sm:px-4 sm:first:pl-0 sm:last:border-r-0">
