...
*声明：本文为MBCT原创分析，数据来源于公开渠道，不构成投资建议。*
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0f172a] py-16">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <Link href={`/${lang}/knowledge`} className="inline-flex items-center text-gray-600 dark:text-muted-foreground hover:text-[#3b82f6] transition-colors mb-8">
          <ArrowLeft className="w-4 h-4 mr-2" />
          {isZh ? '返回前沿导航' : 'Back to Industry'}
        </Link>

        <article className="bg-white dark:bg-[#111827] rounded-2xl border border-gray-200 dark:border-gray-800 overflow-hidden">
          <div className="p-8 border-b border-gray-200 dark:border-gray-800">
            <div className="flex items-center gap-3 mb-4">
              <span className="px-3 py-1 rounded-full bg-[#3b82f6]/10 text-[#3b82f6] text-sm font-medium">{article.tag}</span>
              <span className="text-gray-500 dark:text-gray-500 text-sm">{article.date}</span>
            </div>
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 dark:text-white mb-4">{article.title}</h1>
            <p className="text-xl text-gray-600 dark:text-muted-foreground mb-6">{article.summary}</p>
            <div className="flex flex-wrap items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <button className="p-2 rounded-lg bg-gray-100 dark:bg-muted text-gray-600 dark:text-muted-foreground hover:text-[#3b82f6] transition-colors">
